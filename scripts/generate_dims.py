...
def render_dims(dims_by_ns: dict[str, set[str]]) -> str:
    ...
    lines.append("    def id(self) -> str:")
    lines.append('        """The wire dimension id (same value as ``str(self)``)."""')
    lines.append("        return str(self)")
    lines.append("")
    lines.append("    @property")
    lines.append("    def name(self) -> str:")
    lines.append('        """Alias of :pyattr:`id`. Constants carry no separate display')
    lines.append("        name; localized names come from report metadata")
    lines.append('        (:pyattr:`~dayshape.reports.metadata.DimensionMetadata.display_name`)."""')
    lines.append("        return str(self)")
    lines.append("")
    lines.append("    @property")
    lines.append("    def value(self) -> str:")
    lines.append('        """Alias of :pyattr:`id`, for parity with enum-style access."""')
    lines.append("        return str(self)")
    lines.append("")
